// Audio Homepage Functionality - Load All Products from All Audio Endpoints
...
// DOM elements
const audioProductsShowcase = document.querySelector('.audio-products-showcase');
...
// Function to fetch products from all audio APIs
async function fetchAllAudioProducts() {
    try {
        console.log('Fetching products from all audio APIs...');
        const allProductsArray = [];
        
        // Fetch from all audio APIs in parallel
        const promises = Object.entries(AUDIO_APIS).map(async ([category, apiUrl]) => {
            try {
        ...
        console.log(`Successfully fetched ${allProducts.length} total audio products from all categories`);
        return allProducts;
    } catch (error) {
        console.error('Error fetching all audio products:', error);
        allProducts = [];
        return [];
    }
}
...
function createProductCard(product) {
    ...
    const productCategory = product.category || 'earbuds';
...
function displayProducts() {
    if (!audioProductsShowcase) return;
    
    if (allProducts.length === 0) {
        audioProductsShowcase.innerHTML = `
    ...
    // Display products
    audioProductsShowcase.innerHTML = paginatedProducts.map(product => createProductCard(product)).join('');
...
function addPaginationControls(totalPages) {
    ...
    const existingPagination = document.querySelector('.audio-pagination');
    ...
    audioProductsShowcase.insertAdjacentHTML('afterend', paginationHTML);
...
// Initialize audio homepage when DOM is loaded
document.addEventListener('DOMContentLoaded', async function() {
    console.log('Initializing audio homepage...');
    
    // Show loading state
    showLoading();
    
    // Fetch all products from all audio APIs
    await fetchAllAudioProducts();
    ...
    document.addEventListener('click', function(e) {
        ...
        if (e.target.closest('.price-alert-bell')) {
            ...
            const product = allAudioProducts.find(p => p.id === productId);
    ...
    // Category cards now link directly to audio.html - no JavaScript navigation needed
});
